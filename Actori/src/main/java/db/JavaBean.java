...
public class JavaBean {
	...
	public void connect() throws ClassNotFoundException, SQLException, Exception {
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/actori?useSSL=false", "root", "2005");
	...
	public void adaugaOras(String Nume, String Tara)
		...
		if (con != null) {
			try {
				...
				stmt.executeUpdate("insert into orase(nume,tara) values('" +Nume
		+ "' , '" + Tara + "');");
			} catch (SQLException sqle) {
	...
	public void adaugateatru(String Nume, String Descriere, String Adresa, long Oras)
			throws SQLException, Exception {
		if (con != null) {
			try {
				// creaza un "prepared SQL statement"
				Statement stmt;
				stmt = con.createStatement();
				stmt.executeUpdate("insert into teatre(nume,descriere,adresa,idoras) values('" + Nume +
						"' , '" + Descriere + "', '" + Adresa + "', '" + Oras + "');");
			} catch (SQLException sqle) {
				error = "ExceptieSQL: Reactualizare nereusita; este posibil sa existe duplicate.";
				throw new SQLException(error);
			}
	...
	public ResultSet vedeOrase() throws SQLException, Exception {
	    ResultSet rs = null;
	    try {
	        // Simple query to select the ID and Name of all cities
	        String queryString = "SELECT idoras, nume FROM orase ORDER BY nume";
	        Statement stmt = con.createStatement();
	        rs = stmt.executeQuery(queryString);
	    } catch (SQLException sqle) {
	        error = "SQLException: Interogarea oraselor nu a fost posibila.";
	        throw new SQLException(error);
	    } catch (Exception e) {
	        error = "A aparut o exceptie in timp ce se extrageau orasele.";
	        throw new Exception(error);
	    }
	    return rs;
	}
	...
	public ResultSet vedeTabela(String tabel) throws SQLException, Exception {
		ResultSet rs = null;
		try {
			String queryString = ("select * from `actori`.`" + tabel + "`;");
...
ResultSet.CONCUR_READ_ONLY*/);
	...
	public ResultSet vedeTeatre() throws SQLException, Exception {
		ResultSet rs = null;
		try {
			String queryString = ("select t.idteatru as idteatru"
					+ ",t.nume as nume,"
					+ "t.descriere as descriere,"
					+ "t.adresa as adresa,o.nume as oras,o.tara as tara from teatre t inner join orase o using(idoras)");
		    Statement stmt = con.createStatement(/*ResultSet.TYPE_SCROLL_INSENSITIVE,
ResultSet.CONCUR_READ_ONLY*/);
			rs = stmt.executeQuery(queryString);
		} catch (SQLException sqle) {
	...
	public ResultSet vedeActori() throws SQLException, Exception {
		ResultSet rs = null;
		try {
			String queryString = ("SELECT a.idactor AS idactor, " +      
			           "a.nume AS nume, " +           
			           ...
			           "o.nume AS oras, " +             
			           "o.tara AS tara " +              
			    "FROM actori a, teatre t, orase o " +
			    "WHERE a.idteatru = t.idteatru AND t.idoras = o.idoras" // Correct join conditions
			);
		    Statement stmt = con.createStatement(/*ResultSet.TYPE_SCROLL_INSENSITIVE,
ResultSet.CONCUR_READ_ONLY*/);
			rs = stmt.executeQuery(queryString);
